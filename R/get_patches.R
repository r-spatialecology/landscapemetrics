...
#' @aliases get_patches_old
#' @rdname get_patches_old
#'
#' @export
get_patches_old <- function(landscape, class = "all", directions = 8,
                        to_disk = getOption("to_disk", default = FALSE),
                        return_raster = TRUE) {
    ...
    result <- lapply(X = landscape,
                     FUN = function(x, class, directions, return_raster, to_disk) {
                         ...
                         result <- get_patches_int_old(x_matrix, class, directions)
...
get_patches_int_old <- function(landscape,
                            class,
                            directions) {
    ...
    if (any(class != "all")) {
        ...
        patch_landscape <- lapply(X = class, FUN = function(current_class) {
            ...
            # connected labeling
            rcpp_ccl2(filter_matrix, directions)
            patch_landscape <- filter_matrix
