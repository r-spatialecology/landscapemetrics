    ...
    result <- lapply(X = landscape,
                     FUN = function(x) {
                         ...
                         lsm_class_output(metric = "core_sd",
                                          class = as.integer(names(core_sd)),
                                          value = unname(core_sd))
                     })
...
lsm_c_core_sd_calc <- function(landscape_mat, directions = NULL, consider_boundary = FALSE, edge_depth = 1, resolution = NULL,
    ...
    core_sd <- tapply(core_patch, names(core_patch), stats::sd)

    # return named vector
    stats::setNames(as.double(core_sd), names(core_sd))
}
