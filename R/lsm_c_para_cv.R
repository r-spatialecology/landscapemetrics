...
lsm_c_para_cv <- function(landscape, directions = 8) {
    ...
    result <- lapply(X = landscape,
                     FUN = function(x) {
                         ...
                         para_cv <- lsm_c_para_cv_calc(
                             landscape_mat = landscape_mat,
                             directions = directions,
                             resolution = resolution
                         )

                         lsm_class_output(metric = "para_cv",
                                          class = as.integer(names(para_cv)),
                                          value = unname(para_cv))
                     })
...
lsm_c_para_cv_calc <- function(landscape_mat, directions = NULL, resolution = NULL,
