...
#'
#' @export
lsm_l_contig_mn <- function(landscape) UseMethod("lsm_l_contig_mn")

#' @name lsm_l_contig_mn
#' @export
lsm_l_contig_mn.RasterLayer <- function(landscape) {
    purrr::map_dfr(raster::as.list(landscape), lsm_l_contig_mn_calc, .id = "layer") %>%
        dplyr::mutate(layer = as.integer(layer))
}

#' @name lsm_l_contig_mn
#' @export
lsm_l_contig_mn.RasterStack <- function(landscape) {
    purrr::map_dfr(raster::as.list(landscape), lsm_l_contig_mn_calc,.id = "layer") %>%
        dplyr::mutate(layer = as.integer(layer))
}
...
#' @name lsm_l_contig_mn
#' @export
lsm_l_contig_mn.RasterBrick <- function(landscape) {
    purrr::map_dfr(raster::as.list(landscape), lsm_l_contig_mn_calc, .id = "layer") %>%
        dplyr::mutate(layer = as.integer(layer))
}
...
#' @name lsm_l_contig_mn
#' @export
lsm_l_contig_mn.list <- function(landscape) {
    purrr::map_dfr(landscape, lsm_l_contig_mn_calc, .id = "layer") %>%
        dplyr::mutate(layer = as.integer(layer))
}

lsm_l_contig_mn_calc <- function(landscape) {
