    ...
    result <- lapply(X = landscape,
                     FUN = function(x) {
                         ...
                         lsm_class_output(metric = "para_sd",
                                          class = as.integer(names(para_sd)),
                                          value = unname(para_sd))
                     })
...
lsm_c_para_sd_calc <- function(landscape_mat, directions = NULL, resolution = NULL,
    ...
    para_sd <- tapply(para_patch, names(para_patch), stats::sd, na.rm = TRUE)

    # return named vector
    stats::setNames(as.double(para_sd), names(para_sd))
}
