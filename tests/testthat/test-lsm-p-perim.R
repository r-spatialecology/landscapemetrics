...
test_that("lsm_p_perim results are equal to fragstats", {
    expect_true(all(round(sort(fragstats_patch_landscape_perim) / sort(landscapemetrics_patch_landscape_perim$value) *100) - 100 > 10))
    # expect_true(all(round(sort(fragstats_patch_landscapestack_perim) / sort(landscapemetrics_patch_landscape_stack_perim$value) *100) - 100 > 10))
    # expect_true(all(round(sort(fragstats_patch_augusta_nlcd_perim) / sort(landscapemetrics_patch_augusta_nlcd_perim$value) *100) - 100 > 10))
    # expect_true(all(round(sort(fragstats_patch_podlasie_perim) / sort(landscapemetrics_patch_podlasie_ccilc_perim$value) *100) - 100 > 10))
})
