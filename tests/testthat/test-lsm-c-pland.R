...
test_that("lsm_c_pland equals FRAGSTATS", {
    ...
    fs_landcape <- dplyr::filter(fragstats_class, LID == "landscape", metric == "pland") |> dplyr::pull(value)
    fs_augusta <- dplyr::filter(fragstats_class, LID == "augusta_nlcd", metric == "pland") |> dplyr::pull(value)

    expect_equal(object = sort(lsm_landscape), expected = sort(fs_landcape), tolerance = 0.001)
    expect_equal(object = sort(lsm_augusta), expected = sort(fs_augusta), tolerance = 0.001)
})
