...
Landscape metrics are tools to characterize a landscape. This includes mainly describing the composition and configuration of the landscape. While the composition basically describes how much of the landscape is occupied a certain land cover type, the configuration mainly describes the spatial arrangement to each other of the landcover types. The basic idea of landscape metrics is to condense as much information as possible into single indices.

**landscapemetrics** supports `raster` spatial objects. A raster (sometimes also referred to as grid) contains spatial explicit information by dividing the landscape into discrete cells. Consequently, the cells are the smallest "resolution of information". Because **landscapemetrics** deals with categorical data, each cell has to be assigned to a discrete class. These classes must be positive integer numbers. 

## Levels of landscape metrics
Landscape metrics can be calculated for three different levels (or "scales"). Each level contains information about different aspects of the landscape. Which level is of interest depends on the research question. Often, even a combination of several levels can be useful. Similar, also the chose of which metric to use depends on the research question asked.   

A patch is defined as neighboring cells belonging to the same class i. Hereby, **landscapemetrics** uses the 8-neighbors rule (queen's case) to identify patches. Patch level metrics calculated the metric for every patch in the landscape regardless of the class the patch belongs to. The output will be as long as the number of present patches in the landscape. These metrics are mostly the basis for metrics of the other two levels.

Class level metrics summarise all patches belonging to one class i. These metrics can either be only the "distribution" of patch level metrics of all patches of class i (e.g. the mean) or consider only patches of class i for the calculations of the metric. Regardless of the mathematical background, the output will always be as long as the number of classes is present. Class level metrics are suitable to describe the composition and configuration of the landscape.
...
Core metrics describe the area of patches that are not an edge. These metrics can be interesting for research questions in which e.g. only areas are of interest that are not influenced by neighboring patches of a different class.
...
- The resolution of a cell has to be in **meters**, as the package converts units internally and returns results in a square area for some metrics.
...
that were selected for your research question is straightforward:
```{r}
