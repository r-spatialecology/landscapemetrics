...
All metrics are abbreviated in the result `tibble`. Therefore, we provide a `tibble` containing the full metric names, as well as the class of each metric (`lsm_abbreviations_names`). Using e.g. the `left_join()` function of the `dplyr` package (Hadley *et al.* 2018) one could join a result `tibble` and the abbrevations `tibble`.
...
In `lsm_calculate` this exists as option(`full_name = TRUE`).
