...
**landscapemetrics** re-implements landcsape metrics as they are described in the **FRAGSTATS** software. Therefore, we compared our results with the results of **FRAGSTATS**. In the process, we recognized a few differences between the results. 
...
landscapemetrics_output <- sort(lsm_c_area_cv(landscape)$value)
```

```{r echo = FALSE}
data.frame(
    fragstats_calculated = fragstats_calculated,
    fragstats_output = fragstats_output,
    landscapemetrics_output = landscapemetrics_output
    )
```
...
# Output of landscapemetrics
landscapemetrics_output <- round(sort(lsm_c_area_sd(landscape)$value), 4)
```
...
```{r echo = FALSE}
data.frame(circle_fragstats = circle_fragstats, 
           circle_landscapemetrics = circle_landscapemetrics)
```
...
data.frame(area = area, 
           gyrate_fragstats = gyrate_fragstats, 
           gyrate_landscapemetrics = gyrate_landscapemetrics)
```
...
```{r echo = FALSE}
data.frame(para_fragstats = para_fragstats, 
           para_fragstats_ha = para_fragstats_ha,
           para_landscapemetrics = para_landscapemetrics)
```
