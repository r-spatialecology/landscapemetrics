...
In **landscapemetrics** the depth-of-edge is fixed to one cell. 
In other words, all cells with four adjacent neighbours of the same class are defined as core cells. 
The implementation of the depth-of-edge distance seems to be slightly different in **FRAGSTATS**, leading to slightly different results for core metrics. 

This is a visual aid to help understand how we implemented the core metric. 
The left plot shows all patches of class 1, the right plot the core area (violet cells) in each patch.

```{r}
# All patches of class 1
class_1 <- get_patches(landscape)[[1]]
