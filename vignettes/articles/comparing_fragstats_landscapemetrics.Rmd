...
There are a few important differences between **FRAGSTATS** and **landscapemetrics**
that you should be aware of if you either compare results between the two or use
both of the tools:

- FRAGSTATS rounds after the 4th decimal place
   - That also affects the resolution, if your raster has a resolution with more 
     than 4 decimal places the results for metrics that rely on that are wrong (e.g. AREA, xxx) 

# Specific differences

We thoroughly compared each our implemented landscape metrics against the results
from FRAGSTATS and will give you here a list with why we think there are differences
in the results.

**FRAGSTATS** is unfortunately not an open-source project, so we can't definitely say
some of the metrics have bugs, but we try to show on small examples why we think
there could be some of them.

## Differences on patch level:
...
an error propagation starting there. 
...
The smallest circle around a quadrat should not have the same area as the quadrat,
which is the case in FRAGSTATS:
...
We look at each patch of every class and count the cells that have 4 neighbors
with the value of the specific class.

However, while we can prove that our method works as the description of the core metric
in the FRAGSTATS manual, the results of the FRAGSTATS software itself cannot be replicated.
