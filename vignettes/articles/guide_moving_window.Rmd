...
Following, the result will be a raster with the identical extent as the input, however, each cell now describes the neighborhood in regard to the variability of the chosen metric (Hagen-Zanker 2016). 
...
To analyse an input raster using the moving window approach in **landscapemetrics** we provide the function `window_lsm()`. 
The function allows to specify the neighborhood using a matrix and uses internally the `raster::focal()` function. 
Currently, only landscape level metrics are possible to calculate and can be specified similarly to `calculate_lsm()`.
