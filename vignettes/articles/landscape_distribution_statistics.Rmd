...
```{r, echo=FALSE}
landscapetools::show_landscape(landscape, discrete = TRUE) 
```

## Implemented distribution statistics ## 
The already implemented distribution statistics on class- and landscape-level are indicated by the corresponding suffixes (`_mn` = mean, `_cv` = Coefficient of variation , `_sd` = Standard deviation) and can be calculted without any further calculations needed.
