...
However, also other distribution statistics might be of interest. 
Even though they are not implemented in **landscapemetrics** yet, the integration in the `R` environment allows to calculate them quite easily. 
...
First, we need to calculate the metric of interest on the patch level. 
Additionally, we need the patch area of each patch.
...
Now, we need to join the metric value with the area of each patch, multiply the two values, and calculate the area-weighted mean. 
...
The range equals the minimum value subtracted from the maximum value of the metric of interest for each patch (McGarigal et al. 2012). 
This can be calculated easily using the `min()/max()` function or the `range()` function.
...
## Median 
The median equals the value that divides all (ordered) values into two equal groups.
There is a base `R` function available that can be applied to values grouped by class or all values (landscape-level). 
