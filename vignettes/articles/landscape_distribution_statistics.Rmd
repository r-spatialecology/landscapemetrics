...
First, we need to calculate the metric of interest on the patch level. Additionally, we need the patch area of each patch.
...
Now, we need to join the metric value with the area of each patch, multiply the two values, and calculate the area-weighted mean. To calculate the area-weighted mean on landscape-level, comment out the `dplyr::group_by()`-line.
...
## Median 
The median equals the value that divides all (ordered) values into two equal groups. There is a base `R` function available that can be applied to values grouped by class or all values (landscape-level). 
